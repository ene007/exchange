...
public class MemberTransactionController extends BaseAdminController {
    ...
    //@RequiresPermissions("finance:exchanger:page-query")
    @PostMapping("/exchangeList")
    @AccessLog(module = AdminModule.FINANCE, operation = "所有交易记录MemberTransaction")
    public MessageResult exchangeList(PageModel pageModel) {
        List<Predicate> predicates = new ArrayList<>();
        predicates.add(QMemberTransaction.memberTransaction.type.eq(TransactionType.ACTIVITY_BUY));
        Page<MemberTransactionVO> results = memberTransactionService.joinFind(predicates, pageModel);
        results.getContent().forEach((vo)->{
            Member byExchangeCode = memberService.findMemberByPromotionCode(vo.getExchangeCode());
            if(null != byExchangeCode) {
                vo.setInvitationAccount(byExchangeCode.getRealName());
            }
        });
        return success(results);
    }
