 ...
 * 修改支付信息前必须下架所有的广告，并且没有正在进行中的订单
 功能：该切面类实现了在用户更新支付信息（如支付宝、银行、微信支付信息）之前进行检查，确保用户没有未完成的订单，并且所有广告已经下架。
 技术实现：通过 AOP（面向切面编程），拦截处理支付信息更新的请求，执行前进行业务检查。
 异常处理：如果用户有未完成的订单或未下架的广告，系统会抛出相应的异常并提示用户，阻止继续执行更新支付信息的操作。
