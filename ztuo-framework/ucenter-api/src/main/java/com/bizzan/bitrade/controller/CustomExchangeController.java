...
public class CustomExchangeController extends BaseController {
    ...
    public MessageResult doExchagne(@SessionAttribute(SESSION_MEMBER) AuthMember member, String baseUnit, BigDecimal amount,
                                    BigDecimal price, String exchangeCode) {
        String targetUnit = "BZB";
        //判断邀请账户是否存在
        Member invitationMember = memberService.findMemberByPromotionCode(exchangeCode);
        if(null == invitationMember){
            return error("Exchange code error");
        }
        //不能填你自己的邀请码
        Member member1 = memberService.findOne(member.getId());
        if(member1.getPromotionCode().equals(exchangeCode)){
            return error("You can't fill in your own invitation code");
        }
...
//        if(baseUnit.equals("ETH")) needAmount = amount.divide(ethRate, BigDecimal.ROUND_HALF_DOWN).setScale(8);
        ...
        // 减少基币余额
        walletService.deductBalance(needMW, needAmount);
//                .decreaseFrozen(needMW.getId(), needAmount);
        ...
        memberTransaction.setExchangeCode(exchangeCode);
