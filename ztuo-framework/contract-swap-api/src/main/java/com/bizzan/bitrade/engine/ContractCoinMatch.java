...
public class ContractCoinMatch {
    ...
    public void dealOpenOrder(ContractOrderEntrust order){
        ...
        if (order.getDirection() == ContractOrderDirection.BUY) {
            avaPrice = memberContractWallet.getUsdtBuyPosition().multiply(memberContractWallet.getUsdtBuyPrice()).
                    add(order.getVolume().multiply(openPrice)).
                    divide(memberContractWallet.getUsdtBuyPosition().add(order.getVolume()), 8, BigDecimal.ROUND_DOWN);
            // 更新持仓均价和持仓数量（多仓）
...
//        logger.info("{} 盘面更新：大小-{}", symbol, tradeArrayList.size());
    ...
    public void process(BigDecimal newPrice) {
        ...
        this.processOpenSpotEntrustList(newPrice);     // 2、开仓计划委托处理（止盈止损）
        this.processCloseSpotEntrustList(newPrice);    // 3、平仓计划委托处理（止盈止损）
        this.processCloseEntrustList(newPrice);        // 4、平仓委托处理 （限价）
        this.processOpenEntrustList(newPrice);         // 5、开仓委托处理 （限价）
