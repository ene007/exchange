...
//        return parseResult(returnStr);
    ...
    @Override
    public MessageResult sendInternationalMessage(String content, String nation, String phone) throws Exception {
        SmsSingleSender singleSender = new SmsSingleSender(username, password);
        SmsSingleSenderResult singleSenderResult;
        String smsContent = "[" + this.sign + "]"+"Your verification code is "+content+", which is valid within ten minutes. If it is not operated by yourself, please ignore it.";
        singleSenderResult = singleSender.send(0, nation, phone, smsContent, "", "");
